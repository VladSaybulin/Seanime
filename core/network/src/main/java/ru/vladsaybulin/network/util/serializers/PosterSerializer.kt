...
class PosterSerializer : KSerializer<PosterDto> {
    override val descriptor: SerialDescriptor = buildClassSerialDescriptor("Poster") {
        val stringDescriptor = String.serializer().descriptor
        element("preview", stringDescriptor)
        element("original", stringDescriptor)
    }

    override fun deserialize(decoder: Decoder): PosterDto = decoder.decodeStructure(descriptor) {
